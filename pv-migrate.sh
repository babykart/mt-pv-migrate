...
pv_migrate() {
  case ${MODE} in
    "presync")
      ...
      ${PV_MIGRATE_BIN} migrate --helm-set rsync.extraArgs="--exclude .snapshot" ${SOURCEPVC} ${DESTPVC} -s svc --ignore-mounted --source-namespace ${NAMESPACE} --dest-namespace ${NAMESPACE} || die "SVC PVC migration failed"
    ...
    "migrate")
      ...
      ${PV_MIGRATE_BIN} migrate --helm-set rsync.extraArgs="--exclude .snapshot" ${SOURCEPVC} ${DESTPVC} -s mnt2 --ignore-mounted --source-namespace ${NAMESPACE} --dest-namespace ${NAMESPACE} || die "MNT2 PVC migration failed"
